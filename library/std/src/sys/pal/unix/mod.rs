    ...
    unsafe fn reset_sigpipe(#[allow(unused_variables)] sigpipe: u8) {
        ...
        )))]
        {
            ...
            if let Some(handler) = handler {
                if sigpipe_attr_specified && handler == libc::SIG_IGN {
                    // This process shall ignore SIGPIPE, but child processes
                    // shall get SIG_DFL. As suggested by
                    // https://github.com/rust-lang/rust/issues/62569#issuecomment-1961586025
                    // this is what we get if we install a noop SIGPIPE handler,
                    // since then for child processes the handler will be reset
                    // to SIG_DFL.
                    //
                    // Triggering this code path requires using the unstable
                    // `#[unix_sigpipe = "sig_ign"]` attribute.
                    use crate::mem;
                    use crate::ptr;
                    let current = {
