...
impl<'a, 'tcx> TypeErrCtxt<'a, 'tcx> {
    ...
    fn note_version_mismatch(
        ...
        // itself appears in multiple crate versions that provide an impl for
        // the requested trait. This covers the case where e.g. `From<Foo>` is
        // implemented for `Bar` from v1 but the current code needs it for
        // `Bar` from v2.
