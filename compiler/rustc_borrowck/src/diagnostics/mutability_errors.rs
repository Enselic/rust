    ...
    fn suggest_make_local_mut(&self, err: &mut Diag<'_>, local: Local, name: Symbol) {
        ...
        let amp_mut_sugg = match *local_decl.local_info() {
            LocalInfo::User(mir::BindingForm::ImplicitSelf(_)) => {
                ...
                Some(AmpMutSugg { has_sugg: true, span: decl_span, sugggestion, additional })
            }
...
fn suggest_ampmut<'tcx>(
        ...
        && let Some(ws_pos) = src.find(char::is_whitespace)
    {
        ...
        Some(AmpMutSugg { has_sugg: true, span, sugggestion: " mut".to_owned(), additional: None })
    // if there is already a binding, we modify it to be `mut`
        ...
        Some(AmpMutSugg {
            has_sugg: false,
            span,
            sugggestion: format!("{}mut {}", if decl_ty.is_ref() { "&" } else { "*" }, ty),
            additional: None,
        })
    }
}
