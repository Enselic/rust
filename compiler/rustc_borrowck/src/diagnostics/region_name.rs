...
impl<'tcx> MirBorrowckCtxt<'_, '_, 'tcx> {
    ...
    fn give_name_if_anonymous_region_appears_in_arguments(
        ...
        let (_, span) = self.regioncx.get_user_arg_name_and_span_for_region(
            self.body,
            self.local_names(),
            user_arg_index,
        );
        ...
        let highlight = self
            .get_user_arg_hir_ty_for_highlighting(user_arg_index)
    ...
    fn get_user_arg_hir_ty_for_highlighting(
        &self,
    ...
    fn give_name_if_anonymous_region_appears_in_arg_position_impl_trait(
            ...
            .position(|ty| self.any_param_predicate_mentions(&predicates, ty, region))
        {
            ...
            .position(|ty| self.any_param_predicate_mentions(&predicates, *ty, region))
        {
            let (arg_name, arg_span) = self.regioncx.get_user_arg_name_and_span_for_region(
                self.body,
                self.local_names(),
                arg_index,
            );
